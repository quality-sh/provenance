...
pub const RESERVED_RECORD_IDS: &[&str] = &[
    "__cargo-init", "answer", "assertions", "authoring-changes", "authoring-plans",
    "begin-verification", "boundaries", "check", "claim", "complete-verification",
    "contributions", "coverage", "create", "dictionary", "discussion-containers",
    "dispositions", "docs", "dogfood", "domains", "export", "gaps", "get",
    "graph", "graph-reference", "health", "impact", "import", "init", "list",
    "materialize", "merge-jsonl", "messages", "neighbors", "orphans", "prime",
    "proposals", "questions", "release", "report", "requirements", "resolutions",
    "review", "rules", "schema", "search", "skills", "sources", "statement-checks",
    "submit", "swarm-backtrace", "synthesis-packets", "topics", "trace",
    "traceability", "update", "validate", "verification-bindings", "verification-runs",
    "wiki",
];

/// Refuse a command keyword when a writer assigns a new record ID.
#[rule("rule_record_id_excludes_command_keywords")]
pub fn ensure_record_id_assignable(id: &StableId) -> anyhow::Result<()> {
    if RESERVED_RECORD_IDS.contains(&id.as_str()) {
        anyhow::bail!("reserved record ID {} cannot be assigned", id.as_str());
    }
    Ok(())
}

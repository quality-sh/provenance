use crate::{catalog_cli, cli::Cli, handlers, porcelain};
...
impl Invocation {
    pub fn parse(arguments: Vec<String>) -> anyhow::Result<Self> {
        let Some(word) = grammar::command_word(&arguments) else {
            return Ok(Self::Builtin(Cli::try_parse_from(arguments).unwrap_or_else(|error| error.exit())));
        };
        if word == "search" {
        ...
        if catalog_cli::is_collection(word) {
            ...
            let matches = command.try_get_matches_from(arguments).unwrap_or_else(|error| error.exit());
        ...
        if args.action.as_deref().is_none_or(|action| action == "get") {
            ...
            return Ok(Self::Get(GetInvocation { context, format, input }));
        }
        let action = args.action.as_deref().and_then(provenance_transport::porcelain::Action::parse)
            .ok_or_else(|| anyhow::anyhow!("unsupported target action"))?;
        let kind = args.record_type.as_deref().map(NodeType::parse).transpose()
        ...
        Ok(Self::Target(TargetInvocation { context, format, target: args.target, action, kind, matches }))
    }

...
impl Directory {
    ...
    pub(crate) fn open_child(&self, leaf: &str) -> std::io::Result<Self> {
    ...
    pub(crate) fn create_child(&self, leaf: &str) -> std::io::Result<Self> {
    ...
    pub(crate) fn child_kind(&self, leaf: &str) -> std::io::Result<Option<ChildKind>> {
        child_kind(&self.file, leaf)
    }

    pub(crate) fn rename_no_replace(&self, from: &str, to: &str) -> std::io::Result<()> {
...
fn rename_no_replace_at_raw(
    ...
    let from = CString::new(from.as_bytes()).map_err(|_| ErrorKind::InvalidInput)?;
    let to = CString::new(to.as_bytes()).map_err(|_| ErrorKind::InvalidInput)?;
...
fn rename_no_replace_at(
    ...
    unsafe {
        info.write(FILE_RENAME_INFO {
            ...
            FileNameLength: u32::try_from(name_bytes).map_err(|_| ErrorKind::InvalidInput)?,
            FileName: [0],
        });
        ...
        if SetFileInformationByHandle(
            ...
            FileRenameInfoEx,
            info.cast(),
            u32::try_from(buffer_size).map_err(|_| ErrorKind::InvalidInput)?,
        ) == 0
        {
...
pub(crate) fn rename_no_replace(_from: &Path, _to: &Path) -> std::io::Result<()> {
    unsupported_rename()
}

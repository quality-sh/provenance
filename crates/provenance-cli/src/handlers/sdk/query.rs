...
pub(super) fn handle(operation: Operation, args: QueryArgs) -> anyhow::Result<()> {
    ...
    match operation {
        Operation::Get => {
            let result = queries::get(repo, &scope, super::read_stdin_json()?)?;
            output::print(format, &QueryResponse::new("get", result))
        }
        Operation::Search => {
            let result = queries::search(repo, &scope, super::read_stdin_json()?)?;
            output::print(format, &QueryResponse::new("search", result))
        }
        Operation::Neighbors => {
            let result = queries::neighbors(repo, &scope, super::read_stdin_json()?)?;
            output::print(format, &QueryResponse::new("neighbors", result))
        }
        Operation::Trace => {
            let result = queries::trace(repo, &scope, super::read_stdin_json()?)?;
            output::print(format, &QueryResponse::new("trace", result))
        }
        Operation::Impact => {
            let result = queries::impact(repo, &scope, super::read_stdin_json()?)?;
            output::print(format, &QueryResponse::new("impact", result))
        }
        Operation::Evidence => {
            let result = queries::evidence(repo, &scope, super::read_stdin_json()?)?;
        ...
        Operation::ResolveSymbol => {
            let result = queries::resolve_symbol(repo, &scope, super::read_stdin_json()?)?;

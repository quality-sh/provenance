...
pub(super) fn handle(requested_package: Option<&str>) -> anyhow::Result<()> {
    ...
    add_sdk(&metadata.workspace_root, package)?;
    super::repo::init(
    ...
    .context("failed to initialize Provenance state")?;
...
fn add_sdk(workspace_root: &Utf8Path, package: &CargoPackage) -> anyhow::Result<()> {
    ...
    let status = Command::new("cargo")
        .args(["add", &dependency, "--package", &package.name, "--exact"])
        .current_dir(workspace_root)
        .status()
        .context("failed to run `cargo add` for the Provenance SDK")?;
    anyhow::ensure!(
        status.success(),
        "`cargo add {dependency} --package {} --exact` failed",
        package.name
    );
    Ok(())
}

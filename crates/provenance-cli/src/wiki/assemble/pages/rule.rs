...
impl<'a> Assembler<'a> {
    pub(in crate::wiki::assemble) fn rule_page(&self, rule: &'a Rule) -> RulePage {
        let producing_resolutions = self.query.producing_resolutions(&rule.id);
        let producing_requirements = self.query.producing_requirements(&rule.id);
        ...
        let upstream_requirements: &[&Requirement] = self.query.requirements_behind_rule(&rule.id);
        let sources: Vec<PageLink> = self
            ...
            .filter(|source| {
                self.query
                    ...
                    .iter()
                    .any(|requirement| upstream_requirements.contains(requirement))
            })
            .map(source_link)
            .collect();
        RulePage {

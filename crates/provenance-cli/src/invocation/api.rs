...
fn read_body(input: &str) -> Option<serde_json::Map<String, serde_json::Value>> {
    let text = if input == "-" {
    ...
    let value: serde_json::Value = serde_json::from_str(&text)
        .unwrap_or_else(|error| usage_error(anyhow::anyhow!("--input needs one JSON object: {error}")));
    let object = value
        .as_object()
        .cloned()
...
pub async fn dispatch(args: ApiArgs) -> anyhow::Result<()> {
    crate::porcelain::dispatch_api(&args.common.repo, &args.common.scope, args.common.format(), request(&args)).await
}

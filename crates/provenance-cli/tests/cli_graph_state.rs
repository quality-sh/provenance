...
fn health_rejects_invalid_manifests() {
    for (content, expected) in [
        ("{", "EOF"),
        (&format!("{{\"schema_version\":{},\"scopes\":[]}}", SUPPORTED_SCHEMA_VERSION.0 + 1), "unsupported"),
    ] {
        let directory = tempfile::tempdir().unwrap();
...
fn a_manifest_directory_is_invalid_state() {
    ...
    command(directory.path(), &["health"])
        .failure()
        .stderr(contains("manifest is not a file"));
}

#[test]
fn an_empty_graph_check_keeps_the_repository_read_only() {
    let directory = tempfile::tempdir().unwrap();
    command(directory.path(), &["check", "--graph", "--format", "json"])
        .failure()

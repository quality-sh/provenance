...
fn api_unknown_paths_refuse_with_the_canonical_failure() {
    ...
    let refused = output(&["api", "nowhere/none", "--repo", &repo]);
    assert!(!refused.status.success());
    let printed: Value = serde_json::from_slice(&refused.stderr).unwrap();
...
fn api_rejects_unsupported_methods_and_malformed_flags_before_any_call() {
    ...
    provenance()
        ...
        .assert()
        .failure()
        .stderrpred(predicates::str::contains("invalid value"));

    provenance()
        .args(["api", "sources", "--repo", &repo, "--header", "NoSeparator"])
        .assert()
        .failure()
        .stderrpred(predicates::str::contains("NAME: VALUE"));
    ...
    provenance()
        ...
        .assert()
        .failure()
        .stderrpred(predicates::str::contains("cannot be used multiple times"));

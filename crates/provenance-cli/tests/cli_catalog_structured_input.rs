...
fn canonical_json_flags_express_empty_arrays_and_nested_deltas() {
    let (_directory, repo) = init();
    create_requirement(&repo, "req_target_a");
    ...
    assert!(owner["data"]["depends_on"].is_null());
    ...
    assert!(removed["data"]["depends_on"].is_null());
}
...
fn conflicting_body_sources_are_rejected_without_overwrite() {
    ...
    provenance()
        ...
        .failure()
        .stderr(contains(
            "--name is assigned more than once for body field name",
        ));

...
use provenance_core::{
    ...
    Message, MessageRole, StableId, ThreadParent,
};
...
impl<P: DiscussionPort> crate::Porcelain<P> {
    ...
    pub async fn discussions(&self, input: ListInput) -> Result<DiscussionOutcome, DiscussionError> {
        ...
        let Stamped { result, stamp, freshness_error } = self.port.list(input).await?;
...
fn checked_limit(limit: Option<usize>) -> Result<usize, DiscussionError> {
    ...
    (limit > 0).then_some(limit).ok_or(DiscussionError::InvalidOptions)
}
...
pub fn render_readable(outcome: &DiscussionOutcome) -> String {
    match outcome {
        DiscussionOutcome::List { result, freshness_error, .. } => {
            let mut lines = vec![format!("discussions: {} returned", result.entries.len())];

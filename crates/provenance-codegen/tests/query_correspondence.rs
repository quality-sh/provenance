...
#[test]
fn query_variants_reuse_consolidated_response_components() {
    let (openapi, _) = provenance_codegen::documents();
    ...
    for (contract, consolidated) in [("success", "GetRuleSuccess"), ("failure", "GetRuleFailure")]
    {
        ...
        assert!(
            found.iter().all(|reference| reference.starts_with(&format!(
                "#/components/schemas/{consolidated}"
            ))),
            "{name} must reuse {consolidated} components: {found:?}"
        );
        assert_eq!(
            schemas
                .keys()
                .filter(|component| component.starts_with(name))
                .count(),
            1,
            "{name} must not duplicate nested response components"
        );
    }
}

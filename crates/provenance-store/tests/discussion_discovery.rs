...
use provenance_store::{
    operations::{catalog::{ListDiscussionsV2, Operation, PreparedContext, PreparedRead}, read_policy::ReadPolicy},
...
async fn selected_pages_are_complete_and_bind_status_parent_and_revision() {
    ...
    let page = read_discussion_list(root, &scope(), ReadPolicy::default(), all).await.unwrap().result;
    assert_eq!(page.entries.len(), 3);
    assert!(page.entries.iter().any(|entry| entry.discussion_id == b.discussion_id));
...
async fn conversation_returns_head_and_ordered_bounded_messages() {
    ...
    assert!(read_discussion_conversation(root, &scope(), ReadPolicy::default(), wrong.clone()).await
        .unwrap_err().downcast_ref::<ReadFailure>().is_some_and(|e| *e == ReadFailure::CursorInvalid));
    wrong.discussion_id = entry.discussion_id;
    assert!(read_discussion_conversation(root, &scope(), ReadPolicy::default(), wrong).await
        .unwrap_err().downcast_ref::<ReadFailure>().is_some_and(|e| *e == ReadFailure::CursorRevisionChanged));
}
...
async fn default_list_has_truthful_parent_status_and_bounded_opening() {
    ...
    store.post_thread_message(serde_json::from_value(json!({
    ...
    let entry = store.write_discussion(write("long", json!({
...
async fn page_limits_and_large_messages_are_bounded() {
    let (temp, store) = fixture();
    let root = camino::Utf8Path::from_path(temp.path()).unwrap();
    let entry = store.write_discussion(write("large", json!({
    ...
    assert!(read_discussion_list(root, &scope(), ReadPolicy::default(), list(None, 0)).await.is_err());
    assert!(read_discussion_list(root, &scope(), ReadPolicy::default(), list(None, 201)).await.is_err());
    let mut oversized_cursor = list(None, 1);
    oversized_cursor.cursor = Some("x".repeat(8193));
    assert!(read_discussion_list(root, &scope(), ReadPolicy::default(), oversized_cursor).await
        .unwrap_err().downcast_ref::<ReadFailure>().is_some_and(|e| *e == ReadFailure::CursorInvalid));
}
...
async fn catalog_operation_applies_grants_before_paging() {
    ...
    let mut query = list(None, 1);
    query.allowed_parent_kinds = vec![NodeType::Source];
    let denied = ListDiscussionsV2::run(context.clone(), query).await.unwrap();
    assert!(denied.result.entries.is_empty());
    assert!(!denied.result.has_more);
    assert!(denied.result.next_cursor.is_none());
    let allowed = ListDiscussionsV2::run(context, list(None, 1)).await.unwrap();
...
async fn scope_list_includes_other_parents_and_parent_list_does_not() {
    ...
    let mut input = write("second", json!({"kind":"start","role":"user","body":"second"}));
    ...
    assert!(scope_page.entries.iter().any(|entry| entry.discussion_id == a.discussion_id));
    assert!(scope_page.entries.iter().any(|entry| entry.discussion_id == b.discussion_id));
    let parent_page = read_discussion_list(root, &scope(), ReadPolicy::default(), list(Some(a.parent), 10))
        .await.unwrap().result;
...
async fn concurrent_replies_do_not_split_head_from_messages() {
    ...
    let writer = std::thread::spawn(move || {
        let mut head = first;
        for index in 0..12 {
            head = store.write_discussion(reply(&head, &format!("reply_{index}"))).unwrap();
        }
    });
    for _ in 0..20 {

...
#[tokio::test]
async fn page_limits_and_large_messages_refuse_explicitly() {
    let (temp, store) = fixture();
    let root = camino::Utf8Path::from_path(temp.path()).unwrap();
    let entry = store.write_discussion(write("large", json!({
        "kind":"start", "role":"user", "body":"x".repeat(66_000)
    }))).unwrap();
    ...
    assert!(read_discussion_conversation(root, &scope(), ReadPolicy::default(), conversation).await
        .unwrap_err().downcast_ref::<ReadFailure>().is_some_and(|e| *e == ReadFailure::PageRecordTooLarge));

...
#[test]
fn an_enrolled_thread_alone_cannot_bypass_the_outcome_writer() {
    let (_temp, store) = fixture();
    let entry = store
        .write_discussion(
            serde_json::from_value(json!({
                "scope_id":"default", "parent":{"node_type":"requirement","node_id":"req_a"},
                "request_id":"root", "actor":"ben",
    ...
    assert!(store.create_requirement(serde_json::from_value(json!({
        "scope_id":"default", "id":"req_new", "statement":"The system retains evidence.",
        "status":"discovery", "depends_on":[], "supersedes":[], "origin_thread":entry.thread_id
    })).unwrap()).is_err());

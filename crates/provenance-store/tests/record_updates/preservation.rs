...
async fn target_edit_with_nested_unknown_data_is_refused_without_publication() {
    ...
    clean["sourceRefs"] = json!([{"sourceId":"source_one","clause":"section 1"}]);
    write_rows(
        &fixture,
        "requirements",
        "req.jsonl",
    ...
    let mut loaded = parsed_rows(&fixture, "requirements", "req.jsonl");
    assert_eq!(loaded[0]["description"], "Changed");
    loaded[0]["sourceRefs"] =
    ...
    let error = fixture
        .call(
            "update-requirement",
            json!({"scope_id":"default","id":"req_one","description":"Further"}),
        )
        .await
        .unwrap_err();

    assert_ne!(error["kind"], "unknown_operation");
    assert_eq!(raw_rows(&fixture, "requirements", "req.jsonl"), vec![original]);

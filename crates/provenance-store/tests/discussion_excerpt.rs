...
async fn list_excerpt_counts_characters_across_nul_and_unicode() {
    ...
    let cases = [
        (
            "nul_early",
            format!("abc\0{}", "x".repeat(300)),
            format!("abc\0{}", "x".repeat(236)),
            true,
        ),
        (
            "nul_at_limit",
            format!("{}\0y", "x".repeat(239)),
            format!("{}\0", "x".repeat(239)),
            true,
        ),
        (
            "nul_after_limit",
            format!("{}\0y", "x".repeat(240)),
            "x".repeat(240),
            true,
        ),
        (
            "unicode_nul",
            format!("{}\0{}", "é".repeat(100), "界".repeat(150)),
            format!("{}\0{}", "é".repeat(100), "界".repeat(139)),
            true,
        ),
    ...
    for ((name, body, excerpt, truncated), id) in cases.iter().zip(ids) {
        ...
        if *name == "nul_early" {
            let conversation = read_discussion_conversation(
                root,
                &scope(),
                ReadPolicy::default(),

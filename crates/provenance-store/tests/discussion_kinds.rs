...
fn write_for(kind: NodeType, node: &str, request: &str, declared_by: serde_json::Value) -> WriteDiscussion {
...
fn seed_kinds(store: &StateStore) {
    ...
    store
        ...
        .unwrap();
    store
        .create_rule(serde_json::from_value(json!({
        ...
        .unwrap();
    store
        .create_topic(serde_json::from_value(json!({
        ...
        .unwrap();
    store
        .create_question(serde_json::from_value(json!({
...
fn every_kind_starts_replies_and_resolves_a_concern() {
    ...
    for (kind, node) in every_kind() {
        ...
        let declared_by = if node == "res_award" { json!("maker") } else { json!(null) };
        let started: provenance_core::threads::DiscussionEntry = store
            .write_discussion(serde_json::from_value(json!({
...
fn parent_ownership_and_existence_are_per_kind() {
    ...
    for (kind, node) in [
        ...
        assert!(
            store
                .write_discussion(write_for(kind, node, &format!("wrong_{node}"), json!("other")))
                .is_err(),
    ...
    store
        .write_discussion(write_for(NodeType::Resolution, "res_award", "owned", json!("maker")))
        .unwrap();
    // Claims are work locks, not authorship: topics and questions take no
    // declared owner at all.
    for (kind, node) in [(NodeType::Topic, "topic_award"), (NodeType::Question, "question_award")] {
        assert!(
            store
                .write_discussion(write_for(kind, node, &format!("claimed_{node}"), json!("someone")))
                .is_err(),
    ...
    assert!(store
        .write_discussion(write_for(NodeType::Source, "source_missing", "ghost", json!(null)))
        .is_err());
    // A kind that takes no Discussions refuses before any lookup.
    assert!(store
        .write_discussion(write_for(NodeType::Domain, "domain_missing", "domain", json!(null)))
        .is_err());
}
...
fn legacy_containers_stay_readable_beside_addressed_discussions() {
    ...
    let legacy = store
        .post_thread_message(serde_json::from_value(json!({
            "scope_id": "default",
    ...
    let addressed = store
        .write_discussion(write_for(NodeType::Question, "question_award", "addressed", json!(null)))
        .unwrap();

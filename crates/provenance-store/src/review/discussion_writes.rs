        ...
        self.with_repository_publication(|| {
            ...
            let head = match &input.action {
                ...
                | DiscussionAction::SetStatus { discussion_id, expected_version, .. } => {
                    ...
                    crate::write_error::ensure!(DiscussionMembershipMismatch, head.parent == input.parent, "Discussion parent membership mismatch");
                    crate::write_error::ensure!(DiscussionVersionConflict, head.version == *expected_version, "stale Discussion version");
                    ...
                    crate::write_error::ensure!(DiscussionClosed, canonical.is_some_and(|t| t.id == head.thread_id),
            ...
            match &input.action {
                DiscussionAction::Start { body, .. } | DiscussionAction::Reply { body, .. } => {
                    ...
                    if let Some(head) = &head {
                        crate::write_error::ensure!(DiscussionResolved, head.status == DiscussionStatus::Active, "resolved Discussion refuses replies");
                    }
                }

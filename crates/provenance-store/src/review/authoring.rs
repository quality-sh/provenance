...
impl StateStore {
    ...
    fn save_record(
        ...
        let request_id = authoring_request_id(
            "update-requirement",
            &[&intent_of(&(&update, &relationships))?, expected_etag.as_str()],
        )?;
...
fn retyped(error: anyhow::Error, duplicate: bool) -> anyhow::Error {
    ...
    if duplicate {
        ...
        SourceFailure::wrap(WriteFailure::InvalidUpdate, anyhow::anyhow!(error.to_string()))
    }
}

fn add_delta(add: Vec<StableId>) -> ListEdit {
    ListEdit::Delta {
        add,
        remove: Vec::new(),
    }
...
fn remove_delta(remove: Vec<StableId>) -> ListEdit {
    ListEdit::Delta {
        add: Vec::new(),
        remove,
    }
...
fn authoring_request_id(label: &str, parts: &[&str]) -> anyhow::Result<StableId> {
    ...
    Ok(StableId::new(canonical_digest::sha256(joined.as_bytes()))?)
}

...
impl RequirementRelations {
    ...
    pub(super) fn expand(&self, before: &Requirement) -> anyhow::Result<FinalRelations> {
        ...
        expand_list(&mut final_sets.depends_on, "depends_on", before, &self.depends_on)?;
        expand_list(&mut final_sets.supersedes, "supersedes", before, &self.supersedes)?;
        ...
        expand_cites(&mut final_sets.cites, before, &self.cites)?;
        Ok(final_sets)
    }
}
...
fn sorted_citations(mut refs: Vec<SourceReference>) -> Vec<SourceReference> {
    refs.sort_by(|a, b| a.source_id.as_str().cmp(b.source_id.as_str()).then(a.clause.cmp(&b.clause)));
    refs.dedup();
    refs
}

fn sort_citations(entries: &mut [SourceReference]) {
    entries.sort_by(|a, b| a.source_id.as_str().cmp(b.source_id.as_str()).then(a.clause.cmp(&b.clause)));
}

...
impl StateStore {
    pub fn update_requirement(&self, input: UpdateRequirementInput) -> anyhow::Result<Requirement> {
        with_staged_state(&self.layout, false, |layout| {
            Self::new(layout.clone()).update_requirement_in_staged_state(input)
        })
    }

    pub(crate) fn update_requirement_in_staged_state(
        &self,
        ...
        if let Some(statement) = &input.statement {
            ...
            super::super::statement_policy::ensure_statement_is_writable(&self.layout, statement)?;
        }
        ...
        crate::test_probes::at("requirement_update_record_written")?;

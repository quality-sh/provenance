...
impl StateStore {
    ...
    pub(super) fn list_ideation_landings(
        ...
        read_ideation_landings(self, &shards::ideation_landings_path(&self.layout, scope))
    }
    ...
    pub(super) fn write_ideation_batch(
        ...
        self.mutate_jsonl_records(&path, |landings: &mut Vec<IdeationLandingBatch>| {
            ...
            reject_new_ids(&contributions, &incoming.contributions, |record| &record.id)?;
            reject_new_ids(&synthesis_packets, &incoming.synthesis_packets, |record| &record.id)?;
            reject_new_ids(&proposals, &incoming.proposals, |record| &record.id)?;
            reject_new_ids(&assertions, &incoming.assertions, |record| &record.id)?;
            reject_new_ids(&dispositions, &incoming.dispositions, |record| &record.id)?;

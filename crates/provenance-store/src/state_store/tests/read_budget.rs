...
fn wire_call(operation: &'static str, request: serde_json::Value) -> serde_json::Value {
    json!({
...
async fn member_read(root: &camino::Utf8Path, id: &str) -> Result<serde_json::Value, String> {
    catalog::invoke_with(
        ...
        wire_call("get-source-v2", json!({"id": id})),
...
async fn list_read(root: &camino::Utf8Path) -> Result<serde_json::Value, String> {
    catalog::invoke_with(
        "page-sources-v2",
        SDK_PROTOCOL_VERSION,
        wire_call("page-sources-v2", json!({"limit": 50, "cursor": null})),
        Arc::new(Target(root.clone())),
    )
    .await
    .map_err(|failure| failure.error["kind"].as_str().unwrap_or("").to_string())

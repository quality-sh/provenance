...
async fn reproduction_accepted_oversized_source_fails_member_read() {
    ...
    .await
    .expect_err("current behaviour: the list read refuses the accepted record");
    assert_eq!(list.error["kind"], "page_record_too_large");
}

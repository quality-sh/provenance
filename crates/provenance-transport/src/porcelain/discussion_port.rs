...
use provenance_porcelain::discussion::{
    ConversationInput, DiscussionError, DiscussionPort, ListInput, PortFuture, ReplyInput, StartInput,
};
...
impl DiscussionPort for HostDiscussionPort {
    ...
    fn start(&self, input: StartInput) -> PortFuture<'_, DiscussionEntry> {
        Box::pin(async move {
            let kinds = self.permitted_parent_kinds::<catalog::WriteDiscussionV2>();
...
pub(super) fn is_available(host: &crate::StatementHost) -> bool {
    host.advertises(catalog::ListDiscussionsV2::NAME) &&
        !HostDiscussionPort::new(host.clone()).permitted_parent_kinds::<catalog::ReviewDiscussionsV2>().is_empty()
}

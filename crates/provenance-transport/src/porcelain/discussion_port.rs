...
use provenance_porcelain::discussion::{
    ConversationInput, DiscussionAction, DiscussionError, DiscussionPort, ListAnswer, ListInput, PortFuture, ReplyInput, StartInput,
};
...
impl DiscussionPort for HostDiscussionPort {
    fn list(&self, input: ListInput) -> PortFuture<'_, ListAnswer> {
        Box::pin(async move {
            ...
            Ok(ListAnswer { scope_id: self.scope()?, page })
        })
    }

...
pub(crate) async fn call(
    ...
    let (matched, data, query, headers) = match crate::mcp::mcp_call(route.definition, &Value::Object(arguments)) {
        Ok(call) => call,
    ...
    match crate::routing::invoke(host, &matched, data, query, &headers).await {
        Ok((value, _)) => {
            ...
            result.content = vec![Content::text(render_readable(action, &target, route.kind, &value))];
            result
        }

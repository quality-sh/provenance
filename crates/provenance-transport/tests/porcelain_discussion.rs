...
async fn call(client: &rmcp::service::RunningService<rmcp::RoleClient, ()>, name: &str, args: Value) -> rmcp::model::CallToolResult {
    client.call_tool(CallToolRequestParams::new(name.to_owned()).with_arguments(args.as_object().unwrap().clone())).await.unwrap()
}
...
async fn named_mcp_discussion_actions_share_structured_and_readable_results() {
    ...
    for tool in tools.iter().filter(|tool| names.iter().any(|name| name == tool.name.as_ref())) {
    ...
    assert!(jsonschema::JSONSchema::compile(&json!(write_tool.output_schema.as_ref().unwrap())).unwrap()
        .is_valid(started.structured_content.as_ref().unwrap()));
    let id = receipt["discussion_id"].as_str().unwrap();
    let replay = call(&client, "discuss", json!({
        "parent":{"node_type":"requirement","node_id":"req_shared"},
        "request_id":"request_start", "actor":"ben", "role":"user", "body":"Opening text"
    })).await;
    assert_eq!(replay.structured_content.as_ref().unwrap()["receipt"], *receipt);
    let changed_intent = call(&client, "discuss", json!({
        "parent":{"node_type":"requirement","node_id":"req_shared"},
    ...
    assert_eq!(list.structured_content.as_ref().unwrap()["scope_id"], "default");
    assert_eq!(list.structured_content.as_ref().unwrap()["status"], "active");
    let list_tool = tools.iter().find(|tool| tool.name == "discussions").unwrap();
    assert!(jsonschema::JSONSchema::compile(&json!(list_tool.output_schema.as_ref().unwrap())).unwrap()
    ...
    assert!(readable.contains(id) && readable.contains("scope=default") && readable.contains("limit=50"), "{readable}");
    ...
    let cursor = first_page.structured_content.as_ref().unwrap()["result"]["messages"]["next_cursor"].as_str().unwrap();
    let next_page = call(&client, "discussion", json!({"discussion_id":id,"limit":1,"cursor":cursor})).await;
    assert_eq!(next_page.structured_content.as_ref().unwrap()["result"]["messages"]["entries"][0]["body"], "Second message");
    let stale = call(&client, "reply", json!({
...
async fn denied_parent_grant_is_applied_before_discussion_page_selection() {
    ...
    for (request_id, node_type, node_id) in [
        ...
        if node_type == "source" { source = Some(receipt); }
    }
    ...
    let hidden = call(&client, "discussion", json!({"discussion_id":source.discussion_id})).await;
    assert_eq!(hidden.is_error, Some(true));
    let denied_reply = call(&client, "reply", json!({

...
async fn public_query_wire_keeps_valid_data_and_bounds_a_typed_refusal() {
    ...
    let (status, bytes, value) = call(&host(&valid, false), path).await;
    assert_eq!(status, 200, "{value}");
    assert_eq!(value["data"]["id"], "req_shared");
    assert_eq!(value["data"]["neighbors"].as_array().unwrap().len(), 1);
    ...
    let (status, bytes, value) = call(&host(&oversized, false), path).await;

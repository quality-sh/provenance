...
async fn api_read_returns_the_named_catalog_tool_result_on_the_same_path() {
    let repository = Repository::new("The api shares public paths.");
    repository.all_kinds();
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move { host(&repository).serve_mcp(server_io).await.unwrap() });
    ...
    assert_eq!(shared.structured_content.as_ref().unwrap()["data"]["id"], "source_shared");
...
async fn api_selects_methods_bodies_and_headers_for_one_public_mutation() {
    let repository = Repository::new("The api creates sources.");
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
...
async fn api_refuses_an_unknown_path_with_the_canonical_failure() {
    let repository = Repository::new("The api refuses unknown paths.");
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move { host(&repository).serve_mcp(server_io).await.unwrap() });
    let client = ().serve(client_io).await.unwrap();
...
async fn api_refuses_a_supported_path_with_an_unsupported_method() {
    let repository = Repository::new("The api refuses unsupported methods.");
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move { host(&repository).serve_mcp(server_io).await.unwrap() });
    let client = ().serve(client_io).await.unwrap();
...
async fn api_mutations_keep_the_operation_specific_preconditions() {
    let repository = Repository::new("The api keeps preconditions.");
    repository.all_kinds();
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move {
        StatementHost::with_fixture_access(access(&repository).allow_writes())
    ...
    let no_idempotency = call_api_tool(
        &client,
        json!({"path": "sources", "method": "post", "body": {"id": "source_x", "name": "X", "source_type": "document"}}),
    )
    .await;
    ...
    let no_version = call_api_tool(
        &client,
        json!({"path": "sources/source_shared", "method": "patch", "body": {"reference": "Section 9"}}),
    )
    .await;
...
async fn api_discovery_describes_the_live_catalog_routes() {
    let repository = Repository::new("The api discovers the catalog.");
    repository.all_kinds();
    ...
    // The default fixture grant advertises every read definition and no
    // mutating definition, so the described routes must track that set.
    let advertised = catalog::definitions()
        .iter()
        .filter(|definition| !definition.mutates())
        .count();
    assert_eq!(routes.len(), advertised, "one described route per advertised definition");
...
async fn api_discovery_hides_denied_operations() {
    let repository = Repository::new("The api discovery respects grants.");
    repository.all_kinds();
    let restricted = access(&repository).deny_operation("list-sources");
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move {
        StatementHost::with_fixture_access(restricted)
            .serve_mcp(server_io)
            .await
            .unwrap()
    });
...
async fn host_api_port_and_the_mcp_api_tool_produce_one_result() {
    let repository = Repository::new("The api is one capability.");
    repository.all_kinds();
    let bound = host(&repository);
    ...
    let direct = service.execute_api(ApiRequest::from(arguments)).await;
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move { bound.serve_mcp(server_io).await.unwrap() });
    let client = ().serve(client_io).await.unwrap();
...
async fn host_api_port_maps_canonical_failures_for_direct_callers() {
    let repository = Repository::new("The api classifies refusals.");
    let service = Porcelain::new(HostApiPort::new(host(&repository)));

    let unknown = service
        .execute_api(ApiRequest::from(ApiArguments {

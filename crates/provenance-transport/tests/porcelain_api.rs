...
// TEMPORARY diagnostic: verbatim copy of the working authorization test body.
#[tokio::test]
async fn diagnostic_verbatim_authorization_pattern() {
    let repository = Repository::new("The shared graph is readable.");
    repository.all_kinds();
    let access = access(&repository).allow_writes();
...
async fn api_read_returns_the_named_catalog_tool_result_on_the_same_path() {
    let repository = Repository::new("The shared path is readable.");
    repository.all_kinds();
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
...
async fn api_selects_methods_bodies_and_headers_for_one_public_mutation() {
    let repository = Repository::new("The shared source is created.");
    let (client_io, server_io) = tokio::io::duplex(256 * 1024);
    let server = tokio::spawn(async move {
        StatementHost::with_fixture_access(access(&repository).allow_writes())
            .serve_mcp(server_io)

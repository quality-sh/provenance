...
async fn invalid_statement_calls_have_typed_refusals() {
    ...
    for body in [
        ...
        let (status, actual) = call(&host, "/v7/operations/check-statement", body).await;
...
async fn protocol_and_unknown_operation_refusals_preserve_dispatch_identity() {
    ...
    assert_eq!(
        failure["error"],
        json!({"kind":"protocol_mismatch", "requested":6,"supported":7})
    );
    let (status, failure) = call(&host, "/v7/operations/not-an-operation", "{}").await;
...
async fn metadata_and_statement_calls_leave_fixture_tree_untouched() {
    ...
    assert_eq!(metadata["protocol_version"], 7);
...
async fn oversized_http_body_refuses_before_analyzer() {
    ...
    let (status, refusal) = call(&host, "/v7/operations/check-statement", &body).await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
...
async fn request_admission_bounds_incomplete_bodies_before_decode() {
    ...
    for _ in 0..8 {
        ...
        readers.push(tokio::spawn(async move {
            router
                .oneshot(
                    Request::post("/v7/operations/check-statement")
    ...
    let (status, failure) = call(&host, "/v7/operations/check-statement", "{}").await;
